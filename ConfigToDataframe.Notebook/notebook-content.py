...
def extract_config_info(pdf):
    ...
    for index, row in pdf.iterrows():
        ...
        try:
            config_json = json.loads(config.replace('""', '"'))
            for schema, tables in config_json.items():
                for table, columns in tables.items():
                    for column_info in columns:
                        config_records.append({
                            ...
                            'pattern_name': column_info['pattern_name'],
                            'result': column_info['Result'],
                            'parameter': column_info['parameter']
                        })
        except json.JSONDecodeError as e:
            print(f"Error decoding JSON for object_id {object_id}: {e}")
...
if result_df is not None:
    ...
    spark_result_df.write.mode("append").format("delta").save("abfss://JumpStart_Project@onelake.dfs.fabric.microsoft.com/Silver.Lakehouse/Tables/masking_results")
...
# Extract config information and create a new DataFrame
config_df = extract_config_info(pdf)

# Ensure the data types in the Pandas DataFrame match the schema
config_df['object_id'] = config_df['object_id'].astype(int)

# Define the schema for the config DataFrame
...
# Convert the config DataFrame to a Spark DataFrame and write it to a new Delta table
if config_df is not None:
    spark_config_df = spark.createDataFrame(config_df, schema=config_schema)
    
    # Write the DataFrame as a Delta table
    spark_config_df.write.mode("append").format("delta").save("abfss://JumpStart_Project@onelake.dfs.fabric.microsoft.com/Silver.Lakehouse/Tables/masking_config_results")
    
    # Show the result
    spark_config_df.show()
else:
    print("No config data to display.")
...
if config_df is not None:
    ...
    config_schema = StructType([
        ...
        StructField("pattern_name", StringType(), True),
        StructField("result", StringType(), True),
        StructField("lookup", StringType(), True),
        StructField("parameter", StringType(), True)
    ])
...
# MAGIC %%sql
# MAGIC 
# MAGIC select count(distinct object_id) from masking_config_results;
